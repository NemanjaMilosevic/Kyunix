...
// Creates an IDT entry that does not disable IRQs
#define CREATE_IDT_GATE(idx) \
    create_idt_gate(idx, (uint32_t) &interrupt_handler_##idx,\
                    IDT_TRAP_GATE_TYPE, PL0);

// Creates an IDT entry which automatically disables IRQs when fired
#define CREATE_IRQ_GATE(idx) \
    create_idt_gate(idx, (uint32_t) &interrupt_handler_##idx,\
