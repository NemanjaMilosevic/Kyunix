...
static ps_list_t nonrunnable_pss = { NULL, NULL };
                                           ...
                                           stack_state_t stack)
{
    ...
    if (ms >= SCHEDULER_TIME_SLICE) {
        ...
        pic_acknowledge(0); //PF ZF is? yea i guess so
    }
}
...
int scheduler_add_nonrunnable(ps_t *ps){

    return scheduler_add_process(&nonrunnable_pss, ps);
}
...
void sleep(ps_t *ps){
   ...
   scheduler_add_nonrunnable(ps);
   scheduler_remove_process(&runnable_pss, ps->id, 0);  //removes from pslist
   
   printf("Put to sleep\n");
 
   scheduler_schedule(); //context switch,reschedule
...
scheduler_schedule();
//add process to runnable,print wakeup and reschedule,change context...
