...
printf("Called system read\n");
    ...
    addQueue(ps); //ADD PS TO KEYBOARD QUEU AND PUT IT TO SLEEP,RIGHT okay
    sleep(ps);
 //scheduler_schedule();
//wakeup(ps);
 //scheduler_schedule();
...
//ps->flag=1;
//return kbd_read(vnode,buf,count); 
return vnode->v_op->vn_read(vnode, buf, count);
}


int sys_halt(uint32_t syscall,void *stack){
    UNUSED_ARGUMENT(syscall);
...
static void update_user_mode_registers(ps_t *ps, cpu_state_t cs,
                                       stack_state_t es)
{
    if (!(ps->user_mode.eflags & (1<<9)))
        printf("User mode context has interrupts disabled!\n");
...
// interrupt 0x80 is an interrupt gate so interrupts are already disabled here
